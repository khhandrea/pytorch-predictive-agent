...
class MovingImageEnvironment(gym.Env):
    ...
    def step(self, action: np.ndarray) -> tuple[np.ndarray, float, bool, bool, dict[str, Any]]:
        ...
        # Move up
        if action == 0:
            self._coordinate[1] -= self._agent_speed
        # Move right
        if action == 1:
            self._coordinate[0] += self._agent_speed
        # Move down
        if action == 2:
            self._coordinate[1] += self._agent_speed
        # Move left
        ...
        self._coordinate[0] = max(0, min(self._image.height - self._observation_size[1], self._coordinate[0]))
        self._coordinate[1] = max(0, min(self._image.height - self._observation_size[2], self._coordinate[1]))
    ...
    def _render_frame(self):
        ...
        canvas = pygame.Surface((CANVAS_HEIGHT, CANVAS_HEIGHT))
    ...
    def _get_observation(self, add_noise: bool = True) -> np.ndarray:
        cropped_image = self._image.crop((
            self._coordinate[0],
            self._coordinate[1],
            self._coordinate[0] + self._observation_size[1],
            self._coordinate[1] + self._observation_size[2]
        ))
        ...
        if add_noise:
            noise = np.random.normal(loc=0, scale=10, size=numpy_image.shape)

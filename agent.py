...
class PredictiveAgent:
    ...
    def train(
        ...
        # From t-1 to T
        observations = torch.cat((self._batch_prev_observation, batch['observations']), dim=0)
        # From t-2 to T
        actions = F.one_hot(batch['actions'], num_classes=self._action_space.n).float()
        ...
        inner_states = self._local_networks['inner_state_predictor'](
            torch.cat((detached_features, actions[:-1]), dim=1),
        ...
        if self._hyperparameters['random_policy']:
            ...
            # reward = extrinsic_reward + self._hyperparameters['intrinsic_reward_scale'] * intrinsic_reward
        ...
        # Total loss
        loss = (
            self._hyperparameters['inverse_loss_scale'] * inverse_loss
